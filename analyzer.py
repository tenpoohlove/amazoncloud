...
{{
  ...
  "checklist": [
    {{
      "item": "コンセプトの独自性（既存品と明確に違う点がある）",
      "status": "OK または 要強化",
      "how": "OKなら独自性の根拠を具体的に。要強化なら「〇〇を加えることで差別化できる」と提案（50字以内）"
    }},
    {{
      "item": "ターゲット顧客と痛みの具体性（誰のどんな不満か明確）",
      "status": "...",
      "how": "OKなら対象顧客像と痛みを具体的に。要強化なら「△△な人の□□という不満に絞り込む」と提案"
    }},
    {{
      "item": "一言で伝わる価値提案（キャッチコピーになる強さ）",
      "status": "...",
      "how": "OKならそのキャッチ候補を一文で。要強化なら「〇〇という表現に言い換えると伝わりやすい」と提案"
    }},
    {{
      "item": "レビュー根拠の信頼性（実際の声が裏付けになっている）",
      "status": "...",
      "how": "OKなら引用できる根拠レビューの特徴を。要強化なら「この不満を拾えていない、△△視点を追加」と提案"
    }},
    {{
      "item": "製造・実現可能性（難易度とコストが現実的）",
      "status": "...",
    ...
    {{
      "item": "緊急性・限定性の設計可能性（今すぐ支援する理由が作れる）",
      "status": "...",
      "how": "OKなら具体的な訴求軸を。要強化なら「先着〇名・期間限定〇%OFFなど限定要素を設定する」と提案"
    }},
    {{
      "item": "競合との比較優位性（既存品より明らかに優れている点）",
      "status": "...",
      "how": "OKなら比較で勝てる具体的なポイントを。要強化なら「〇〇という点を強化しないと埋没する」と提案"
    }},
    {{
      "item": "開発ストーリーの作りやすさ（共感できる背景が描ける）",
      "status": "...",
